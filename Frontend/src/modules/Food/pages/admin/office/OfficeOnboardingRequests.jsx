...
export default function OfficeOnboardingRequests() {
  ...
  const handleApprove = async (id) => {
    try {
      await adminAPI.put(`/food/admin/office-companies/${id}/approve`);
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              ...
              Office Onboarding Requests
            </h1>
            <p className="text-sm text-gray-500 mt-1">
              {selectedRequest ? `Reviewing request for ${selectedRequest.legalName}` : 'Review and approve new office registrations.'}
            </p>
          </div>
        </div>
      </div>
