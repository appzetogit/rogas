...
function NewDeliveryDashboard() {
  ...
  const handleConfirmPickup = () => {
    setOrders(
      (prev) => prev.map((o) => o.id === "order_maria_kitchen" ? { ...o, status: "picked_up" } : o)
    );
    setStops(
      (prev) => prev.map((s) => {
        if (s.orderId === "order_maria_kitchen" && s.type === "P") {
          return { ...s, status: "COMPLETED" };
        }
        if (s.orderId === "order_maria_kitchen" && s.type === "D") {
  ...
  const handleConfirmDelivered = (cashCollected) => {
    setOrders(
      (prev) => prev.map((o) => o.id === "order_maria_kitchen" ? { ...o, status: "delivered" } : o)
    );
    setStops(
      (prev) => prev.map((s) => s.orderId === "order_maria_kitchen" ? { ...s, status: "COMPLETED" } : s)
    );
    setStats((prev) => ({
      ...prev,
  ...
  const handleSubmitFailure = (report) => {
    ...
    setStops(
      (prev) => prev.map(
        (s) => s.orderId === "order_maria_kitchen" ? { ...s, status: "FAILED" } : s
      )
    );
    setCurrentScreen("route");
  };
