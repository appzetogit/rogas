...
export const ViewSupportTicketV2 = () => {
  ...
  useEffect(() => {
    ...
    const handleUpdate = (updated) => {
      if (updated.complaintId === ticketId) {
        ...
        toast.info(t("Ticket status updated by Admin"));
      }
    };
  ...
  if (!ticket) return <div className="p-20 text-center text-gray-400 font-bold uppercase tracking-widest h-screen">{t("Ticket Not Found")}</div>;
  ...
  return (
    <div className="min-h-full bg-transparent font-poppins pb-20">
      ...
      <div className="bg-white px-4 py-5 flex items-center gap-4 sticky top-0 w-full z-50 shadow-sm border-b border-gray-100">
        ...
        <h1 className="text-xl font-black text-[#2B2B2B] uppercase tracking-tight">{t("Ticket Info")}</h1>
      </div>
      ...
      <div className="pt-24 px-4 space-y-6">
         ...
         <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center justify-between">
            <div className="space-y-1">
               <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{t("ID Reference")}</p>
               <h3 className="text-lg font-black text-[#2B2B2B]">#{ticket.ticketId || t("Pending")}</h3>
            </div>
         ...
         <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 space-y-4">
            <div className="space-y-1">
               <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{t("Subject")}</p>
               <h4 className="text-sm font-black text-[#2B2B2B]">{ticket.subject}</h4>
            </div>
            <div className="space-y-1 pt-4 border-t border-gray-100">
               <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{t("Detail Description")}</p>
         ...
         {ticket.proofPhotos && ticket.proofPhotos.length > 0 && (
           <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 space-y-3">
             <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{t("Attached Screenshot")}</p>
             <div className="flex flex-wrap gap-3 pt-1">
               {ticket.proofPhotos.map((photo, index) => (
                 ...
                 >
                   <img 
                     src={photo} 
                     alt={t("Screenshot {{index}}", { index: index + 1 })}
         ...
         <div className="bg-white rounded-3xl p-6 shadow-sm border border-[#1F7A63]/20 flex items-start gap-4">
            ...
            <div className="space-y-2">
               <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{t("Support Response")}</p>
               {ticket.adminResponse ? (
                 ...
                 <p className="text-xs text-[#2B2B2B]/60 font-bold leading-relaxed italic">
                   {t("Our support team is currently reviewing your ticket. You'll receive a notification once there is an update.")}
                 </p>
               )}
               {ticket.respondedAt && (
                 <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">
                   {t("Updated {{date}}", { date: new Date(ticket.respondedAt).toLocaleString() })}
                 </p>
               )}
            </div>
         </div>
         ...
         <div className="mt-10 flex flex-col items-center justify-center opacity-20 gap-4">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-center text-[#2B2B2B]">{t("AppZeto Support Fleet")}</p>
         </div>
      </div>
    </div>
  );
